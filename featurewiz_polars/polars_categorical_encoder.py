...
class Polars_CategoricalEncoder(TransformerMixin):  # Class name updated to V2
    ...
    def __init__(
        ...
        if self.encoding_type not in ["woe", "target", "ordinal", "onehot"]:
            raise ValueError(
                f"Invalid encoding_type: '{encoding_type}'. Must be 'woe', 'target', 'ordinal' or 'onehot'."
            )
        if self.handle_unknown not in ["value", "error"]:
            raise ValueError(
                f"Invalid handle_unknown: '{handle_unknown}'. Must be 'value' or 'error'."
            )
        if (
            self.encoding_type == "woe"
            and self.unknown_value is not None
            and not isinstance(self.unknown_value, float)
        ):
            raise ValueError(
                f"unknown_value for WoE encoding must be a float or None, got {type(self.unknown_value)}"
            )
    ...
    def fit(self, X, y=None):
        ...
        if y is None:
            raise ValueError(
                "Target 'y' must be provided for CategoricalFeatureEncoder."
            )
        ...
        if isinstance(self.categorical_features, list):
            ...
            for col in categorical_cols:
                if col not in X.columns:
                    raise ValueError(
                        f"Your input categorical_features column '{col}' not found in your DataFrame."
                    )
        else:
            if self.categorical_features == "auto":
                ...
                categorical_cols = [
                    ...
                    if X[col].dtype in [pl.Categorical, pl.Utf8, pl.Boolean]
                ]
            else:
                raise ValueError(
                    f"Your input for categorical_features must be either auto or a list of features."
                )
        ...
        if self.encoding_type == "woe":
            ...
            if y.dtype == pl.String:
                ...
                global_stats = (
                    ...
                    .select(
                        pl.col(target_name)
                        .filter(pl.col(target_name) == positive_class)
                        .count()
                        .alias("global_events"),
                        pl.col(target_name)
                        .filter(pl.col(target_name) == negative_class)
                        .count()
                        .alias("global_non_events"),
                    )
                    .collect()
                )
        ...
        if self.encoding_type == "onehot":
            ...
            self.one_hot_categories_ = {
                col: X[col].unique().sort().to_list() for col in categorical_cols
            }

        elif self.encoding_type == "woe":
            if self.model_type == "regression":
                print(
                    "Weight of evidence encoding cannot be used in Regression. Please try using another encoding instead. Returning"
                )
                return self
            ...
            if y.dtype == pl.String:
                ...
                for feature in categorical_cols:
                    ...
                    epsilon = 1e-7
                    woe_mapping = (
                        category_stats.with_columns(
                            (
                                (
                                    (pl.col("events") + epsilon)
                                    / (total_events + 2 * epsilon)
                                ).log()
                                - (
                                    (pl.col("non_events") + epsilon)
                                    / (total_non_events + 2 * epsilon)
                                ).log()
                            ).alias("woe")
                        )
                        .select(feature, "woe")
                ...
                for feature in self.categorical_feature_names_:
                    ...
                    woe_mapping = (
                        event_count.with_columns(
                            non_event_count=pl.col("count") - pl.col("event_count")
                        )
                        .with_columns(
                            event_rate=(pl.col("event_count") + 0.5)
                            / (total_event + 1.0),
                            non_event_rate=(pl.col("non_event_count") + 0.5)
                            / (total_non_event + 1.0),
                        )
                        .with_columns(
                            (pl.col("event_rate") / pl.col("non_event_rate"))
                            .log()
                            .alias("woe")
                        )
                        .select(feature, "woe")
            ...
            for feature in categorical_cols:
                ...
                target_mapping = (
                    dfx.to_pandas().set_index(feature).to_dict()["target_mean"]
                )
        ...
        if self.encoding_type == "onehot":
            ...
            if self.encoding_type == "ordinal":
                ...
                for feature in self.encoders_:
                    ...
                    expr = pl.col(feature).replace(
                        encoder_dict, default=self.unknown_value
                    )
                ...
                for feature in self.categorical_feature_names_:
                    if feature in self.encoders_:
                        ...
                        if self.handle_unknown == "value":
                            ...
                            X_transformed = X_transformed.with_columns(
                                pl.col(feature)
                                .replace(encoding_map, default=unknown_val)
                                .alias(feature)
                            )
                            ...
                            if any(
                                cat not in encoding_map
                                for cat in X_transformed[feature].unique()
                            ):
                                unknown_categories = [
                                    cat
                                    for cat in X_transformed[feature].unique()
                                ...
                                raise ValueError(
                                    f"Unknown categories '{unknown_categories}' encountered in feature '{feature}' during transform."
                                )
                            X_transformed = X_transformed.with_columns(
                                pl.col(feature).replace(encoding_map).alias(feature)
                            )
                    else:
                        ...
                        if self.handle_unknown == "value":
                            ...
                            X_transformed = X_transformed.with_columns(
                                pl.lit(self.unknown_value).alias(feature)
                            )
                        elif self.handle_unknown == "error":
                            raise ValueError(
                                f"Feature '{feature}' was specified as categorical but not seen during fit."
                            )
    ...
    def _one_hot_transform(self, X):
        ...
        for feature in self.categorical_feature_names_:
            ...
            if self.handle_unknown == "error":
                ...
                if unseen.len() > 0:
                    raise ValueError(
                        f"Found unknown categories in feature '{feature}': {unseen.unique().to_list()}"
                    )

            # Create one-hot encoded columns
            encoded = X.select(pl.col(feature)).to_dummies(
                feature, drop_first=self.drop_first, separator="_"
            )
            # Merge encoded columns back to main DataFrame
            X_transformed = pl.concat([X_transformed, encoded], how="horizontal").drop(
                feature
            )
