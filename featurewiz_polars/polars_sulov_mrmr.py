...
class Sulov_MRMR(BaseEstimator, TransformerMixin):
    ...
    def sulov_mrmr(self, X: pl.DataFrame, y: pl.Series) -> List[str]:
        ...
        if self.verbose:
            ...
            if len(features_to_remove) > 0:
                ...
                print(
                    f"SULOV removed Features ({len(features_to_remove)}): {feats_joined}"
                )
            else:
    ...
    def _calculate_mis(
        self, X: pl.DataFrame, y: pl.Series, features: List[str], cat_cols: List[str]
    ) -> Dict[str, float]:
    ...
    def _calculate_correlations(self, X: pl.DataFrame) -> pl.DataFrame:
        ...
        return (
            corr_matrix.with_columns(feature_a=pl.Series(numeric_df.columns))
            .unpivot(
                index="feature_a", variable_name="feature_b", value_name="correlation"
            )
            .filter(
    ...
    def _adaptive_removal(
        self, corr_pairs: List[tuple], mis_scores: Dict[str, float]
    ) -> set:
        ...
        return {
            f
            for f, count in removal_candidates.items()
            if mis_scores[f] < 0.5 * max_mis
        }

    def _enforce_min_features(
        self, remaining: List[str], mis_scores: Dict[str, float]
    ) -> List[str]:
        ...
        min_score = max(
            0.1 * q75, 1.20 * scores[scores > 0].min()
        )  # At least 10% of 75th percentile
    ...
    def recursive_xgboost(self, X: pl.DataFrame, y: pl.Series) -> List[str]:
        ...
        tiers = {
            "high": [
                f
                for f, imp in zip(sorted_features, base_importances)
        ...
        if total_features < 20:
            iter_limit = max(
                3, int(total_features / 3)
            )  # set this number high to avoid selecting unimportant features
            top_ratio = 0.25
        else:
            iter_limit = max(
                3, int(total_features / 5)
            )  ## you can tighten the number of chunks with more features
            top_ratio = 0.15

        top_num = int(
            total_features * top_ratio
        )  ## set this number low to avoid selecting too many
        ...
        for i in range(0, total_features, iter_limit):
            ...
            importances = pd.Series(
                self._get_model_importances(model), index=chunk_features
            )
            max_imp = importances.max()
            if max_imp == 0.05:
                continue  # Skip chunks with no importance
            ...
            knee_location = KneeLocator(
                range(len(importances)), importances.values, curve="convex"
            ).knee
        ...
        if self.verbose > 0:
            ...
            print("#" * 77)
            print(
                "####### Classic feature selection: Maximum Relevance process explained ######"
            )
            print("#" * 77)
        ...
        if total_features > 30:
            ...
            if len(votes) > min_features:
                ...
                cutoff = KneeLocator(
                    range(len(votes)), votes.values, curve="convex"
                ).knee
            else:
                cutoff = len(votes) // 2
        ...
        if self.verbose:
            print(
                f"Voted: {len(voted_features)} features based on votes >= 1: \n{voted_features}"
            )

        ### This final next step is needed to boost selection performance!
        if self.verbose:
            print(
                f"Initial: {len(initial_features)} feature(s) selected from full model fit are: \n{initial_features}"
            )

        ### Putting voted features first works well here too ##
        final_features = list(
            OrderedDict.fromkeys(voted_features + initial_features).keys()
        )
        ...
        if self.verbose:
            print(
                f"Final: {len(final_features)} features reflect union of best features from all angles:\n{final_features}"
            )
    ...
    def _get_upper_triangle(self, corr_matrix: pl.DataFrame) -> pl.DataFrame:
        ...
        feature_indices = pl.DataFrame(
            {"feature": features, "idx_b": pl.int_range(0, len(features))}
        )

        # Add row indices (idx_a) and melt
        upper_triangle = (
            corr_matrix.with_columns(
                feature_a=pl.Series(features), idx_a=pl.int_range(0, len(features))
            )
            .melt(
    ...
    def _get_upper_triangle_old(self, corr_matrix: pl.DataFrame) -> pl.DataFrame:
        ...
        corr_with_names = corr_matrix.with_columns(feature_a=pl.Series(features_list))

        # Melt to long format (feature_a, feature_b, correlation)
        long_format = corr_with_names.melt(
            id_vars="feature_a", variable_name="feature_b", value_name="correlation"
        )

        # Create index-based comparison for upper triangle
        return (
            long_format.with_columns(
                pl.col("feature_a")
                .map_elements(lambda x: features_list.index(x))
                .alias("idx_a"),
                pl.col("feature_b")
                .map_elements(lambda x: features_list.index(x))
                .alias("idx_b"),
            )
            .filter(pl.col("idx_a") < pl.col("idx_b"))
            .drop(["idx_a", "idx_b"])
        )

    def _coerce_datasets(self, X, y) -> tuple[pl.DataFrame, pl.Series]:
        ...
        if not isinstance(X, pl.DataFrame):
            if type(X) == tuple:
                ...
                print(
                    "Input 'X' is a numpy array. It must be a Polars DataFrame. Returning as-is..."
                )
                return (X, y)
            else:
    ...
    def _get_model_importances(self, full_model):
        ...
        if "randomforest" in str(full_model).split("(")[0].lower():
            ...
            raise ValueError(
                "Cannot recognize your input estimator. Please use one of the following: xgboost, lightgbm, catboost, randomforest"
            )
        return model_imp
    ...
    def recursive_xgboost_with_validation(
        self,
        X: pl.DataFrame,
        y: pl.Series,
        num_runs: int = 3,
        ...
        for run_idx in range(num_runs):
            ...
            X_train_pl, X_val, y_train_pl, y_val = polars_train_test_split(
                X,
                y,
            ...
            base_params = {
                ...
                "max_depth": 4,
                "random_state": self.random_state
                + run_idx,  # slightly different random state for each run
                "n_jobs": -1,
            }
            ...
            for i in range(0, total_features, iter_limit):
                ...
                model.set_params(**base_params)
                model = self._fit_different_models(
                    model, X_train_pl[chunk_features], y_train_pl
                )
                ...
                if self.verbose > 0:
                    print(
                        f"Features selected in iteration #{iteration} are:\n", selected
                    )

                weight = 1 + (i / (total_features * 2))  # 1x to 1.5x weight
                for feat in selected:
                    feature_votes[feat] += weight
                    run_selected_features.add(
                        feat
                    )  # Add to set of features selected in this run

                iteration += 1

            all_selected_features_runs.append(
                run_selected_features
            )  # Store features selected in this run
        ...
        if self.verbose:
            print(
                f"Multi-run: {len(multirun_features)} features under consideration from multiple runs: \n{multirun_features}"
            )
        ...
        if self.verbose:
            print(
                f"Initial: {len(initial_features)} features chosen from all-features model fit: \n{initial_features}"
            )
